<template>
  <div>
    ...
    >
      ...
      <el-card
        class="box-card"
        style="width: 70%; margin: 0 auto; margin-top: 10px"
      >
        <!-- <p v-html='spot.content'></p> -->
        <p class="ql-snow ql-editor" v-html="spot.spotContent"></p>
      </el-card>

      <el-card class="box-card commentCard">
        <el-input
          placeholder="请发布有价值的评论，博客评论不欢迎灌水，良好的社区氛围需大家一起维护。"
          v-model="contentCopy"
        >
          <el-button
            slot="append"
          ...
          <div class="one" v-for="item in replies" :key="item.id">
            ...
            <div>
              ...
              >
                <el-button type="text"
                  ><span
                    style="font-size: 8px; color: rgb(173, 173, 173)"
                    @click="replyComment(item.id, item.commentName)"
                    >回复</span
                  ></el-button
                >
              </div>
                ...
                <div>
                  ...
                  >
                    <el-button type="text"
                      ><span
                        style="font-size: 8px; color: rgb(173, 173, 173)"
                        @click="replyComment(cItem.id, cItem.commentName)"
                        >回复</span
                      ></el-button
                    >
                  </div>
                  <div class="date">
                    <i class="el-icon-date"></i> {{ cItem.createTime }}
                  </div>
                </div>
              </div>
            </el-card>
...
export default {
  ...
  methods: {
    getData() {
      ...
      getSpotByIdAPI(id).then(
        function (res) {
          // console.log(res)
          this.spot = res.data.data.spotById;
        }.bind(this)
      );
      getCommentByBelongIdAPI(id, 1).then(
        function (res) {
          console.log(res);
    ...
    replyComment(id, name) {
      if (this.userInfo == null || this.userInfo == "") {
        ...
        this.replyForm.belongId = this.spot.id;
        this.replyForm.userId = this.userInfo.id;
      }
    },
    deleteComment(id) {
      this.$confirm(
        "删除该评论会将所有该评论的回复一并删除, 是否继续?",
        "提示",
        {
